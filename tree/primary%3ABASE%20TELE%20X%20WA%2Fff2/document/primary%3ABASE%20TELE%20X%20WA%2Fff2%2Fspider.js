...
const mquote = {
    key: {
        ...
        fromMe: false,
        id: 'SPIDER_XD_STYLISH',
        participant: '0@s.whatsapp.net'
    },
    message: {
        conversation: "ꜱᴘɪᴅᴇʀ-xᴅ ᴏᴘᴛɪᴍɪᴢᴇᴅ ʙʏ ɢᴀᴀʀᴀ ᴛᴇᴄʜ 🕷️"
    }
};
...
const isAdmins = isGroup ? groupAdmins.includes(nowsender) : false;
    ...
    if (m.quoted) {
        try {
            ...
            const caption = quotedMsg.caption || `*ꜱᴘɪᴅᴇʀ-xᴅ ꜱᴀᴠᴇ* 🕷️`;
...
`;
 ...
 newsletterName: '𝑺𝑷𝑰𝑫𝑬𝑹 𝑿𝑫',
 serverMessageId: 125
 },
 externalAdReply: {
 title: "ꜱᴘɪᴅᴇʀ xᴅ ᴀssɪsᴛᴀɴᴛ",
...
case 'repo': {
 ...
 const repoMsg = `🕷️ *${toSmallCaps("spider xd repository")}*

*┌─⊷ ${toSmallCaps("project details")}*
*│ ◈ ${toSmallCaps("name")} : Spider XD V1*
*│ ◈ ${toSmallCaps("author")} : Gaara Tech*
...
> *${toSmallCaps("get the latest version and documentation on our official website below")}* ⚡`;
 ...
 body: { text: repoMsg },
 footer: { text: "ꜱᴘɪᴅᴇʀ-xᴅ ᴏᴘᴛɪᴍɪᴢᴇᴅ ʙʏ ɢᴀᴀʀᴀ ᴛᴇᴄʜ" },
 nativeFlowMessage: {
 buttons: [
 {
 name: "cta_url",
 buttonParamsJson: JSON.stringify({
 display_text: toSmallCaps("open spider xd web"),
 ...
 gaara.reply("https://spiderxd.vezxa.com");
 }
}
break;
...
case 'ai': {
 try {
 const axios = require('axios');
 if (!text) return m.reply(`*Sᴘɪᴅᴇʀ AI* 🕷️\n\nHello! I am Spider AI, created by *Gaara Tech*. How can I assist you today?`);

 // Visual reaction while processing
 await sock.sendMessage(m.chat, { react: { text: "🔍", key: m.key } });

 // System Prompt: English Instructions for Identity & Behavior
 const systemPrompt = `Your name is SPIDER AI. Your creator is Gaara Tech. 
 You are a highly intelligent and helpful assistant. 
 Always respond in the same language the user speaks. 
 Never mention Jeeves or Faa. If asked who you are, say you are SPIDER AI, an automated bot by Gaara Tech.`;
 ...
 .replace(/Jeeves AI/gi, "SPIDER AI")
 .replace(/Faa/gi, "Gaara Tech");
...
> ${toSmallCaps("spider-xd system settings")}`;
...
case 'webdl': {
    try {
        ...
        gaara.reply(toSmallCaps("cloning and injecting spider xd signature..."));
        ...
        let caption = `
*╭──────────────╼*
*│ 🌐 ${toSmallCaps("spider xd web cloner")}*
*╰──────────────╼*
...
> *${toSmallCaps("powered by gaara tech")}*`.trim();
...
case 'channel': {
    try {
        ...
        let caption = `
*╭──────────────╼*
*│ 📢 ${toSmallCaps("spider xd channel stalk")}*
*╰──────────────╼*
...
case 'rbg': {
    try {
        ...
        await sock.sendMessage(gaara.chat, {
            image: { url: apiUrl },
            caption: `✨ *${toSmallCaps("spider xd removebg")}*\n\n> *${toSmallCaps("arrière-plan retiré")}*`
        }, { quoted: mquote });
...
case 'ppcp': {
    try {
        ...
        await sock.sendMessage(gaara.chat, { 
            image: { url: res.male }, 
            caption: `♂️ *${toSmallCaps("spider xd male")}*` 
        }, { quoted: mquote });

        // Envoi de la version Femme
        await sock.sendMessage(gaara.chat, { 
            image: { url: res.female }, 
            caption: `♀️ *${toSmallCaps("spider xd female")}*` 
        }, { quoted: mquote });
...
case 'take': case 'steal': case 'swm': {
    try {
        ...
        const finalPackname = packname.trim() || "𝚂𝙿𝙸𝙳𝙴𝚁-𝚇𝙳";
...
case 'telestick': case 'tgsticker': {
    try {
        ...
        for (let i = 0; i < limit; i++) {
            ...
            const stickerWithMeta = await addExif(buffer, "𝚂𝙿𝙸𝙳𝙴𝚁-𝚇𝙳", "𝙶𝙰𝙰𝚁𝙰-𝚃𝙴𝙲𝙷");
        ...
        await sock.relayMessage(gaara.chat, {
            viewOnceMessage: {
                message: {
                    interactiveMessage: {
                        header: {
                            title: `*${toSmallCaps("spider xd settings")}*`,
                            hasMediaAttachment: false
                        },
                        body: { text: autoReactHeader + autoReactBody },
                        footer: { text: "ꜱᴘɪᴅᴇʀ-xᴅ ᴀᴜᴛᴏʀᴇᴀᴄᴛ ᴍᴀɴᴀɢᴇʀ 🕷️" },
                        nativeFlowMessage: {
                            buttons: [
                                {
                                    name: "quick_reply",
...
*🕷️ ${toSmallCaps("generated by spider-xd")}*`;
    ...
    try {
        ...
        await sock.relayMessage(gaara.chat, {
            viewOnceMessage: {
                message: {
                    interactiveMessage: {
                        header: { title: `*${toSmallCaps("spider xd manager")}*`, hasMediaAttachment: false },
                        body: { text: openMsg },
                        footer: { text: "ꜱᴘɪᴅᴇʀ-xᴅ ᴏᴘᴛɪᴍɪᴢᴇᴅ" },
                        nativeFlowMessage: {
                            buttons: [{
                                name: "quick_reply",
                                buttonParamsJson: JSON.stringify({
...
case 'allcase': {
    try {
        ...
        let menu = `🕷️ *${toSmallCaps("spider xd cases list")}* 🕷️\n\n`;
        cases.forEach((c, i) => {
            menu += `*${i + 1}.* ${c}\n`;
        });
...
case 'getcase': {
    try {
        ...
        const getMsg = `📦 *${toSmallCaps("spider xd extractor")}*
...
> *${toSmallCaps("optimized by gaara tech")}*`;
...
case 'botclone': {
    try {
        ...
        if (result && result.code) {
            ...
            const pairMsg = `✅ *${toSmallCaps("spider xd pairing")}*
            ...
            viewOnceMessage: {
                message: {
                    interactiveMessage: {
                        ...
                        body: { text: resultMsg },
                        footer: { text: "𝑺𝑷𝑰𝑫𝑬𝑹 𝑿𝑫 𝑷𝑶𝑾𝑬𝑹𝑬𝑫 𝑩𝒀 𝑮𝑨𝑨𝑹𝑨 𝑻𝑬𝑪𝑯 🕷️🕸️" },
                        nativeFlowMessage: {
                            buttons: [
                                {
                                    name: "cta_copy",
...
break;
    ...
    try {
        ...
        const aliveMsg = `*${toSmallCaps("spider xd is active")}* 🚀

> ${toSmallCaps("the most powerful and stable bot developed by gaara tech")}

*┌─⊷ ${toSmallCaps("spider xd alive")}*
...
*${toSmallCaps("type")} ${prefix}${toSmallCaps("menu to display commands")}*`;
        ...
        console.error(e);
        gaara.reply(toSmallCaps("spider xd system is currently online"));
    }
}
break;
...
case 'test': {
    try {
        ...
        const bodyText = toSmallCaps("powered by gaara tech");
        const systemInfo = toSmallCaps("spider-xd test");
...
> *${bodyText}*`;
        await sock.sendMessage(gaara.chat, {
            ...
            contextInfo: {
                externalAdReply: {
                    title: toSmallCaps("spider xd test"),
            ...
            await sock.relayMessage(gaara.chat, {
                viewOnceMessage: {
                    message: {
                        interactiveMessage: {
                            header: {
                                title: `*${toSmallCaps("spider xd configuration")}*`,
                                hasMediaAttachment: false
                            },
                            body: { text: modeMsg },
                            footer: { text: "ꜱᴘɪᴅᴇʀ-xᴅ ᴏᴘᴛɪᴍɪᴢᴇᴅ ʙʏ ɢᴀᴀʀᴀ ᴛᴇᴄʜ" },
...
case 'owner': {
    try {
        ...
        const ownerName = "ɢᴀᴀʀᴀ ᴛᴇᴄʜ";
        
        // --- CONSTRUCTION DE LA VCARD (CORRIGÉE) ---
        const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n' 
            + 'FN:' + ownerName + '\n'
            + 'ORG:Spider XD Developer;\n'
...
> *${toSmallCaps("feel free to contact the owner for any help or bugs regarding spider xd")}* 🕷️`;
        ...
        gaara.replyContact("Gaara Tech", "Spider XD Developer", "50940986014");
    }
}
break;
